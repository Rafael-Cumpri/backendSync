...
client.on('disconnected', (reason) => {
    ...
    client.initialize(); // Re-inicializa o cliente em caso de desconexão
});
...
async function getSemanal() {
    try {
        ...
        const query = `
           SELECT h.*, u.nome FROM historico h JOIN usuarios u ON h.funcionario = u.nif WHERE h.data_fim IS NULL;
        `;
...
function formatMessage(data) {
    ...
    data.forEach(row => {
        ...
        message += `Usuário: ${row.nome}, Data de Início: ${row.data_inicio}, Dia da Semana: ${dayOfWeek}\n`;
    });
